...
for (i in seq_along(displayvals)) {
    ...
    to.use <- collected[collected[,2] <= meanval,]
    to.use <- to.use[order(to.use[,1]),]
...
In the HVG data set (result of running https://github.com/MarioniLab/BiocWorkflow2016/workflow.Rmd), an average count of 1 corresponds to a maximum variance of around 1.
...
```{r}
to.use <- collected[collected[,2] <= 1,]
...
```{r}
to.use <- collected[collected[,2] <= 0.1,]
plot(to.use[,1], to.use[,3],xlim=c(0, 1), ylim=c(0, 1))
...
That probably doesn't matter much, though, given that we filter on the means in `voom` prior to _limma_ anyway.
