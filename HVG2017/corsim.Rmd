...
1. There is a lower bound on the expression values, derived from a count of zero.
2. An expression value at the lower bound cannot drop below the bound upon correction, by definition.
3. An expression value at the lower bound cannot increase upon correction, as this suggests expression where there is no evidence for it.
4. Thus, all expression values at the lower bound should attain the same corrected value (i.e., residual).

The constant value is defined as some value below the smallest residual across all observations.
This assumes that a corrected value derived from a non-zero count can never be smaller than a corrected value derived from a zero count.
The exact value does not matter for rank-based methods, as long as it is clearly lower than other residuals.

So, to this end, we can have a look at the performance of _scran_'s correlation calculator with a lower bound.
This avoids the problems with overstatement of the correlation.
The _p_-values are unlikely to be accurate here, but the normality assumption in simulating the observations is probably a bigger problem.
