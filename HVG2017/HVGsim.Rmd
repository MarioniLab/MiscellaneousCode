...
First, the variance-of-logs method picks up the non-outliers first:
...
Whereas, for the CV^2^ methods, the outliers dominate the ranking:

```{r}
out <- technicalCV2(counts, is.spike, sf.cell=1, sf.spike=1)
head(out[order(out$cv2 - out$trend, decreasing=TRUE),], 20)
out <- improvedCV2(counts, is.spike, sf.cell=1, sf.spike=1)
head(out[order(out$cv2 - out$trend, decreasing=TRUE),], 20)
```

In general, count-based models put too much weight on large outliers, such that they cannot be easily removed from the HVG list.
This is another motivation for the use of the variance-of-logs method.
The reduced biological component for outliers from the variance-of-logs method allows them to be effectively removed by the additional `> 0.5` filtering.
Additional filtering is necessary as the null is false when outliers are present and will be rejected with enough cells, regardless of the statistical approach.

# Wrapping up

To conclude, the `technicalCV2` method almost always just gives more noise at a FDR of 5%, without any appreciable increase in power.
If you throw in the bizarre trend fit and the testing procedure, it's clear that the choice is between `improvedCV2` and `trendVar`.
